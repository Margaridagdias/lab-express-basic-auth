...
router.post('/signup', (req, res) => {
  ...
  if (!regex.test(password)) {
    ...
    return;
      ...
      .then((user) => {
        if(user) { // user !== (different than) undifined
          res.render('auth/signup', {
          errorMessage: 'The username alredy exists'
      });
      return;
    }
  });
      ...
      .catch((error) => {
        ...
        res.status(500);
        res.render('auth/signup', {
        errorMessage: 'Username and password need to be unique'
        });
      };
    });
  });
